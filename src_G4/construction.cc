...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
	...
	////////////////////
		...
		G4double DET_L = 0.3*mm;    
		DET_L=DET_L/2.;    
		G4double FR4_L =0.8/2.*mm;    

		
		// Default GeomConfig=11 -> line 447
		if(GeomConfig==1 || GeomConfig==3  ){ // Geometries with direct LYSO construction throguh Geant4 (Bar)
		...
		else if (GeomConfig==2) // Geometries with direct LYSO construction throguh Geant4 (Tile)
		{
		...
		solidGlue = new G4Box("solidGlue", RESIN_W*mm, RESIN_H*mm+0.2*mm*G4UniformRand(), GLUE_L*mm);
			...
			GCgmsh->CreateG4LYSO(scintillator,logicWorld); // Create LYSO in Gmsh
			...
			solidGlue = new G4Box("solidGlue", RESIN_W*mm+0.2*mm, Glue_Y*mm+0.194*mm+0.2*mm*G4UniformRand(), GLUE_L*mm); // create glue
			if (ArgsPass->GetReflSiPM()==1){ // make a reflector in between the resin and glue in cases where the LYSO and SiPM areas are modified (not default)
		...
		solidDetector = new G4Box("solidDetector", DET_TX*mm, DET_T*mm, DET_L); // create SiPM box
		G4RotationMatrix rotm  = G4RotationMatrix();
		G4Transform3D tr = G4Translate3D(0, +0.5*mm+DET_T-RESIN_H, RESIN_L*mm) * G4Rotate3D(rotm) ;
		G4Box* box = new G4Box("Box_1",RESIN_W*mm, RESIN_H*mm, RESIN_L*mm+DET_L); // rotated SiPM box
		G4int nSiPM=16; // number of SiPMs
		
		if(GeomConfig==1 || GeomConfig==2 || GeomConfig==11 ){ // Define resin optional cutouts btw SiPMs to increase light collection reducing CT
		...
		} else if (GeomConfig==3){ // not under use
		...
		}else if (GeomConfig==4){ // not under use
	...
	////////////////////
		...
		// create the scintillator logic depending on GC
		...
		if (GeomConfig==4){logicLYSOCover = new G4LogicalVolume(LYSOCover_Sub, scintillator, "logicLYSOCover");} // not under use

		if (ArgsPass->GetReflSiPM()==1){ // follow up in logic reflector btw glue and resin, optional
		...
		if (ArgsPass->GetnResinMach()>0 || GeomConfig==3){// follow up in resin-SiPM cutouts, optional
...
//  //
//////////////////////
    ConstructSDandField(); // Self created function for the creation of a sensitive detector, see line 891
...
logicDetector->SetSensitiveDetector(sensDet);
    ...
    void MyDetectorConstruction:: DefaultValues() // Defines default geometry values depending on Argsval
{
    ...
    void MyDetectorConstruction:: DefineMessengers() // Messengers for geometry modification through the G4 gui. Requires restarting the run/reread the geometry
{
